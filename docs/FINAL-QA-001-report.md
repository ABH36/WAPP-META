...
### API-Contract QA (partially live-tested this session, partially cross-referenced from the existing regression suite)
...
| **TD-041**             | **OPEN — SECURITY-RELEVANT — ACCEPTED FOR CURRENT RELEASE ONLY IF EXPLICITLY RISK-ACCEPTED** | Per the Architect's explicit Final QA Review decision (2026-08-17): **this status is deliberately distinct from every other row in this table.** Not closed, not routinely "accepted" — it remains open, tracked, and requires its own separately approved remediation initiative. See "TD-041 — Required Risk Acceptance" below for the full, standalone treatment this item is given, separate from the rest of this disposition sweep. |
...
## TD-041 — Required Risk Acceptance

Per the Architect's explicit Final QA Review decision (2026-08-17), TD-041 is given its own standalone treatment, separate from the routine disposition sweep above:

**What it is:** a backend authorization gap, not a missing feature. `PermissionsGuard.canActivate` only checks `level !== NONE` — it does not distinguish `VIEW_ONLY` from `FULL`. `Administrator` holds `BILLING_ACCESS` at `VIEW_ONLY`, and the frontend correctly hides Subscription Upgrade/Downgrade/Cancel actions from that role — but the backend route itself has no equivalent server-side check, so a direct API call (bypassing the UI entirely) from an Administrator-role user would still succeed today.

**Status: OPEN. Not closed. Not silently fixed. Not classified as cosmetic or documentation debt.** This is a genuine, real, backend authorization concern that requires its own separately approved remediation initiative — Final QA's role here is to surface it clearly for an explicit accept/reject decision, not to resolve it.

**This item is the primary accepted release risk for `v1.0.0-rc.2`.** Production deployment of this candidate requires explicit, recorded risk acceptance of TD-041 as a precondition (see the Production Deployment Gate below) — it is not waived through by this report's own READY WITH ACCEPTED RISKS recommendation on its own.
...
**Primary accepted release risk:**

# TD-041

**Candidate:** `v1.0.0-rc.2`, commit `b651eae456a23f1b14c2b2dbed76d0d1351e3de8`

This recommendation is **conditional on explicit acceptance of TD-041** (above) before production deployment — Final QA surfaces this risk clearly; it does not itself waive it. Secondary, non-defect accepted risks are the coverage-scope honesty notes threaded through this report: the full 7-role tenant permission matrix and the Platform-side functional journey were not freshly live-tested this session (existing automated test coverage stands, but wasn't independently re-verified by hand this time — see each domain section above for the explicit historical-evidence-vs-current-execution distinction), and production smoke tests remain entirely untested pending a separate release-window authorization, per Decision 3.

**Final QA itself does not authorize production deployment.** This recommendation is for Release Candidate readiness only.

## Production Deployment Gate

Per the Architect's explicit instruction, before production deployment may begin, the following ten items must each be explicitly confirmed by the Architect/Product Owner (not inferred from this report, not self-certified by Final QA):

1. Explicit risk acceptance recorded for TD-041.
2. Final production commit/version confirmed.
3. Confirmation that the Release Candidate (`v1.0.0-rc.2` / `b651eae`) remains the intended artifact/source for the production release.
4. `docs/RELEASE-CHECKLIST.md` Section A completed for the actual release.
5. Production environment configuration confirmed (`.env` on the VPS matches required-variable list).
6. MongoDB Atlas backup checklist item manually confirmed by the responsible operator (Final QA cannot verify this programmatically — see `docs/ADR-PHD-008-production-deployment-rollback-strategy.md`).
7. Deployment window confirmed.
8. Production smoke-test authorization explicitly granted for this specific deployment (per Decision 3 — no standing authorization exists).
9. Rollback target confirmed (the last known-good production tag/commit, to roll back to if this deployment fails).
10. Observability confirmed operational against the actual production environment (not just this session's local/Docker evidence).

**Final QA status:** ✅ EXECUTED · ✅ VERIFIED WITH DOCUMENTED LIMITATIONS · ✅ RECOMMENDATION: READY WITH ACCEPTED RISKS
**Not yet:** ❌ Production Deployed · ❌ Production Released

Production deployment is the next separate, distinctly-authorized phase — it does not begin automatically as a consequence of this recommendation.
