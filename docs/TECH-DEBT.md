...
**Status:** Closed (2026-08-17, FINAL-QA-001 — Release Validation & Production Readiness)
...
**How this was actually closed (2026-08-17, FINAL-QA-001):** discovered to have a much bigger consequence than "3 known lint warnings" while executing the first-ever real GitHub Actions run of PHD-001 Volume-4's CI workflows — `pnpm lint` (`pnpm -r lint`) exits non-zero on these 3 findings unconditionally, and every downstream CI job (`Test`, `Build`, `E2E`, all 3 Docker verification jobs) depends on `lint-typecheck` passing. This means **CI's lint gate had likely never been capable of showing a fully green run since this entry was first raised** (2026-08-13) — invisible until now because no real GitHub Actions execution had ever happened before (PHD-001 Volume-4 could only verify CI's constituent commands directly, `act` being unavailable locally). Given the small, already-scoped, mechanical nature of the fix this entry itself described 4 days earlier, closing it was approved as part of Final QA's CI verification rather than deferred again: `refresh-cookie.service.spec.ts`'s `jest.fn()` calls given explicit generic type parameters matching their declared `FakeResponse` interface; `platform-compliance.service.spec.ts`'s `as unknown as never[]` double-cast removed outright (the receiver already accepted the plain array's inferred type); `platform-analytics-governance.e2e-spec.ts`'s two raw-string `"SESSION_TIMEOUT"` comparisons replaced with the actual `GovernancePolicyKey.SESSION_TIMEOUT` enum member (imported from the schema), and its untyped `res.body.data` access given an explicit `ApiSuccessResponse<unknown[]>` cast matching the pattern already used elsewhere in the same file. Verified: `pnpm --filter @wapp/api lint`/`typecheck` both clean, the two affected unit-test files and the full `platform-analytics-governance.e2e-spec.ts` suite (22/22) pass, and the complete 729/729 test suite re-run clean afterward.
...
## TD-065 — `/api/metrics` returned JSON-wrapped output, unparseable by a real Prometheus scraper, discovered and closed during FINAL-QA-001's live Release Candidate validation

**Raised and closed same day:** 2026-08-17 (FINAL-QA-001, Release Validation & Production Readiness — live RC validation, doubt-policy resolution)

**Status:** Closed (2026-08-17)

**What:** `MetricsController.getMetrics()` (PHD-001 Volume-2, frozen) declared `@Res({ passthrough: true })` and returned the Prometheus text-exposition string as its function return value. The app's global `ResponseInterceptor` (`APP_INTERCEPTOR`, applied to every route with no exemption) unconditionally wraps every controller return value in `{success, message, data}` — `passthrough: true` still runs the full interceptor pipeline on the return value, so the response body was `{"success":true,"message":"Request successful","data":"# HELP wapp_http_requests_total...\n..."}`, not raw Prometheus text. The `Content-Type` header was set correctly (`text/plain; charset=utf-8; version=0.0.4`, via a direct `response.setHeader()` call, unaffected by the interceptor), which is exactly why this was never caught by casually checking the header or the auth-gating status codes (401/401/200) — only inspecting the actual response _body_ revealed the mismatch. A real Prometheus instance scraping this endpoint would receive content its text-format parser cannot parse at all — this observability surface has likely never actually been scrapeable by real monitoring infrastructure since PHD-001 Volume-2 built it.

**Why this went undetected across two prior volumes' own verification passes:** `metrics.controller.spec.ts`'s existing unit test called `controller.getMetrics(response)` directly and asserted on the method's _return value_ — calling a controller method directly bypasses Nest's real HTTP pipeline (interceptors included) entirely, so a test shaped this way structurally could not have caught this: the method's own return value was always the correct string, only the interceptor's transformation of it (which the test never exercised) was wrong. PHD-001 Volume-2's own runtime verification checked `/api/metrics`'s status-code gating (401/401/200) but not its body content against a real Prometheus text-format parser. Only found here because Final QA's live RC validation checked the actual response body over real HTTP, not just status codes or a direct-call unit test.

**Why fixed immediately rather than just documented:** Same reasoning already approved for TD-057/TD-064 earlier the same session — small, well-understood, standard NestJS fix (switching to full manual response control for this one route), not a design question, and this is a genuine, previously-undiscovered defect in a production monitoring surface, not a lint nitpick — leaving a broken `/metrics` endpoint in the release candidate would mean PHD-001 Volume-2's entire metrics investment was silently non-functional for its actual purpose.

**What the fix was:** `getMetrics()` changed from `@Res({ passthrough: true })` + `return metricsText` to `@Res()` (no passthrough — full manual response control, Nest's own documented way to opt a single route out of the interceptor pipeline for the body) + `response.send(metricsText)`, returning `Promise<void>`. `metrics.controller.spec.ts` rewritten to assert on `response.send(...)`'s own argument instead of the method's return value, so it now actually tests the thing that was broken. Verified: typecheck/lint clean, the updated unit test passes, and — critically — the fix was confirmed against a real running container over real HTTP (`curl .../api/metrics` now returns a body starting directly with `# HELP wapp_http_requests_total...`, no JSON envelope), not just at the unit-test level that missed it the first time. Full 729/729 regression re-run clean.

**Trigger that closed it:** discovered during FINAL-QA-001's live Release Candidate validation (Step 5/6e) — the release candidate was rebuilt as `v1.0.0-rc.2` to carry this fix, since `v1.0.0-rc.1`'s tag remains immutable per Decision 2.
