...
export class StorageService {
  ...
  /**
   * PRD-006 Volume-4 §4.3 (Data Export) — the one deliberate exception to
   * this class's "never handles file bytes" posture: an export buffer is
   * generated by WAPP itself (CSV/Excel/JSON built server-side from the
   * workspace's own data), not an untrusted client upload, so SEC-016's
   * "never proxy arbitrary client bytes through our body parser" rationale
   * doesn't apply here — there's no client upload to bypass.
   * `resource_type: "raw"` is Cloudinary's setting for non-image files.
   */
  async uploadBuffer(
    buffer: Buffer,
    folder: string,
