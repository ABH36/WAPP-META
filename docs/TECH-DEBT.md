...
**Closing this out looks like:** a dedicated Cross-Tenant Support Access initiative that adds: a new `PlatformPermission` tier gating support access separately from every other Platform permission (so `MANAGE_WORKSPACE_STATUS` doesn't implicitly grant it), a genuinely audited "Support Session" concept (start/end timestamps, the specific tenant and data scope accessed, and — per §6's "completely isolated" framing — probably a requirement that the action is visible to the tenant, not silent), and only then whatever read (or, for Break-Glass, write) surface into CRM/Billing/Communication's tenant-scoped repositories the approved design calls for. This is a natural candidate for its own ADR once scoped, given the security/compliance surface involved.

**Trigger to revisit:** a real support-escalation need that the existing Platform Administration surface (Workspace Registry status changes, Announcements, Dashboard aggregates) cannot address — e.g., a customer support ticket that requires a Platform Administrator to see inside a specific tenant's data to diagnose it — surfaced and explicitly approved as a new planning document, the same process every other module in this codebase has gone through.
